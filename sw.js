...
self.addEventListener('fetch', e => {
  ...
  if (/api\.open-meteo\.com|nominatim\.openstreetmap\.org|kp\.gfz\.de|kp\.gfz-potsdam\.de|services\.swpc\.noaa\.gov|\/kp\.json/.test(e.request.url)) { e.respondWith(fetch(e.request)); return; }
  if (/\/seed\.json/.test(e.request.url)) {
    e.respondWith(
      ...
      .catch(() => caches.match(e.request).then(hit => hit || new Response('{}', {headers:{'Content-Type':'application/json'}})))
    );
    return;
  }
